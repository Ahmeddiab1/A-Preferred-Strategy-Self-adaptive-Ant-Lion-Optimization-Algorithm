...
% This function creates random walks 这个函数创建随机游动

function [RWs]=Random_walk_around_antlion(Dim,max_iter,lb, ub,antlion,current_iter)
if size(lb,1) ==1 && size(lb,2)==1 %Check if the bounds are scalar 检查边界是否是标量
    lb=ones(1,Dim)*lb;
    ub=ones(1,Dim)*ub;
end
...
% Dicrease boundaries to converge towards antlion 双酶边界向安特利昂方向收敛
